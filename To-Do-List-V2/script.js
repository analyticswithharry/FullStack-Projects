...
let taskChart = new Chart(document.getElementById('taskChart'), { // Creates a Chart.js bar chart
    type: 'bar', // Sets chart type to bar
    data: { // Defines chart data
        labels: ['Low', 'Medium', 'High'], // Labels for priority categories
        datasets: [{ // Chart dataset
            label: 'Tasks by Priority', // Dataset label
            data: [0, 0, 0], // Initial data for low, medium, high priorities
            backgroundColor: ['#22c55e', '#f59e0b', '#ef4444'], // Colors for low, medium, high
            borderColor: ['#16a34a', '#d97706', '#dc2626'], // Border colors
    ...
    options: { // Chart options
        ...
        scales: { // Configures axes
            y: { // Y-axis settings
                ...
                title: { // Y-axis title
                    display: true, // Shows title
                    text: 'Number of Tasks' // Title text
                }
            }
        },
        plugins: { // Plugin settings
            legend: { // Legend settings
                display: true, // Shows legend
...
function updateStatsAndChart() { // Defines function to update stats and chart
    ...
    const tasksData = JSON.parse(localStorage.getItem('tasks')) || []; // Gets tasks from storage
    const lowCount = tasksData.filter(task => task.priority === 'low').length; // Counts low-priority tasks
    const mediumCount = tasksData.filter(task => task.priority === 'medium').length; // Counts medium-priority tasks
    const highCount = tasksData.filter(task => task.priority === 'high').length; // Counts high-priority tasks
    taskChart.data.datasets[0].data = [lowCount, mediumCount, highCount]; // Updates chart data
